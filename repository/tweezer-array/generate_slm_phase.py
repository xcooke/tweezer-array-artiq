...
class GenerateSLMPhase(EnvExperiment):
    def build(self):
    ...
    @rpc
    def auto_set_exposure(self, stop_if_saturated=True, overexpose_factor=1.0):
        ...
        if stop_if_saturated and max_pixel_value > 1021:
            raise ValueError(f"At minimum exposure time the camera saturated")

        exposure = overexpose_factor * exposure * goal_max_pixel_value / max_pixel_value

        self.set_camera_exposure(exposure)

        max_pixel_value = np.max(self.get_camera_hardware_image())

        print(f"Auto set camera exposure to {exposure:.2e}s, max pixel value = {max_pixel_value}")
    ...
    def get_everything_ready(self):
        print("Make sure SLM path is off!!!")
    ...
    def generate_slm_phase_pattern(self, aom_range, n):
        ...
        self.auto_set_exposure(stop_if_saturated=False, overexpose_factor=7.0)

        print("Now generating phase pattern")

        print(aom_range)

        print(n)

        folder_name = self.slm_cam.generate_slm_phase_pattern(aom_range, n, 2048)
    ...
    def run(self):
        ...
        frequencies = [[-8.0, -8.0], [-8.0, 8.0], [8.0, -8.0], [8.0, 8.0]]
        # pregenerate a coordinates list of 4 times (0, 0)
        coordinates = [(0.0, 0.0)] * 4
        i = 0
        for freq in frequencies:
            x = freq[0]
            y = freq[1]
            self.set_aom_spot(x, y, 1.0, 1.0)
            coordinates[i] = self.process_aom_range_spot()
            self.core.break_realtime()
            i += 1

        self.core.break_realtime()

        self.generate_slm_phase_pattern(coordinates, 5)
        self.core.break_realtime()